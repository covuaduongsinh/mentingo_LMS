...
| Đợt    | PR                  | Nội dung                                                                                                                                                                 |
| ------ | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **L0** | _(đang triển khai)_ | Tài liệu khảo sát clean-room `docs/research/lila/` (6 file: cleanroom-policy, architecture-teardown, data-model, feature-matrix, subsystem-notes, roadmap) — không code. |

**Việc cần làm tiếp:** hoàn tất Đợt L0 rồi lần lượt L1 (bàn cờ tương tác + board editor) → L2 (Study/Chapter) → L3 (Glicko-2 + puzzle thích ứng) → L4 (chơi trực tuyến) → L5 (lớp học cờ) → L6 (ghép cặp hàng loạt + giải đấu) → L7 (nhập môn) → L8 (phân tích điểm mạnh-yếu) → L9 (cộng đồng cờ) → L10 (tường thuật giải đấu), theo đúng thứ tự và phạm vi trong kế hoạch đã duyệt.
