...
| **Đợt 10**      | _(xem mục dưới)_ | GDPR export + ẩn danh hóa (thu hẹp phạm vi — chỉ hạng mục này trong 8 hạng mục "vận hành & tuân thủ" ban đầu): xuất JSON toàn bộ dữ liệu 1 user (hồ sơ + học tập + cộng đồng), ẩn danh hóa tài khoản (xóa PII + thu hồi token, giữ nguyên chứng chỉ/thống kê), chặn ẩn danh hóa admin cuối cùng của tenant. Spec: `docs/specs/gdpr-user-data-business-spec.md`. |
...
**Việc cần làm tiếp:** verify + commit + push + tạo PR + merge Đợt 10 — đây là đợt cuối cùng trong roadmap "còn thiếu so với LearnHouse" đã lên kế hoạch. 7 hạng mục còn lại của Đợt 10 gốc (custom domain, menu builder, landing page builder, custom scripts, đồng tác giả khóa học, khóa bài học theo nhóm, xuất user CSV, SEO mặc định theo tenant) và bàn phân tích cờ cộng tác (hoãn từ Đợt 8) vẫn còn trong `docs/research/learnhouse/05-roadmap.md` như các đợt tiềm năng cho tương lai, nếu có nhu cầu.
