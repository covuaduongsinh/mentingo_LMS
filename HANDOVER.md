...
| **Đợt 9**       | _(xem mục dưới)_ | Webhook đi ra (thu hẹp phạm vi — 6 sự kiện cốt lõi thay vì ~40, không làm tích hợp Zapier riêng): endpoint webhook admin quản lý (URL/sự kiện/bật-tắt/secret mã hoá), ký HMAC-SHA256, gửi qua hàng đợi BullMQ với retry backoff, SSRF guard tái dùng từ link-preview, nút gửi thử + log 50 lần gần nhất. Spec: `docs/specs/outbound-webhooks-business-spec.md`. |
...
**Việc cần làm tiếp:** verify + commit + push + tạo PR + merge Đợt 9, rồi tiếp tục Đợt 10 (vận hành & tuân thủ: GDPR export/anonymize, custom domain, menu builder, landing page builder, custom scripts, đồng tác giả khóa học, khóa bài học theo nhóm, xuất user CSV, SEO mặc định theo tenant) theo đúng `docs/research/learnhouse/05-roadmap.md`.
