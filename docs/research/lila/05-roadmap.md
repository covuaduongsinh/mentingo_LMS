...
| **C2** | #34 merged | Di trú dữ liệu từ `groups` (backfill idempotent) + alias tương thích ngược cho 5 endpoint `chess-class` cũ                                                                                                                                                                                                               |
| **C3** | #36 merged | Học sinh trong lớp: mời/accept/decline, tạo đơn/hàng loạt, hồ sơ, archive/reset mật khẩu/graduate/đóng/chuyển lớp, thao tác hàng loạt                                                                                                                                                                                    |
| **C4** | #38 merged | Bảng tin (`wall`), thông báo toàn lớp (tái dùng pipeline `announcements` qua `sourceType="classroom"`), tự đăng ký giáo viên (`become-teacher`, env-gated), trang admin (`/admin/classrooms`), cron auto-archive 30 ngày (dry-run mặc định, `CLASSROOM_AUTO_ARCHIVE_ENABLED`)                                            |
| **C5** | #39 merged | Nối lớp với LMS: gán/gỡ khóa học (mirror `group_courses`, không refactor `CourseService`), chia sẻ Study cờ (tái dùng `chess_study_members`), mời hàng loạt vào giải đấu qua `chess_tournaments.classroom_id` (mirror `groupId`, không auto-join). "Giao bài tập" xác nhận không cần code riêng — tự động theo khóa học. |
