...
- C4–C8 chưa triển khai tại thời điểm C3 kết thúc.
- Kiểm tra kid-mode DM đầy đủ qua tầng API với 2 tài khoản managed thật (không chỉ SQL) — nay đã khả thi vì C3 có endpoint tạo học sinh managed, nên làm sớm ở C4.
...
- Namespace `chessClass.*` vẫn thiếu hoàn toàn ở `pl`/`de`/`lt`/`cs`/`es` (nợ kỹ thuật có từ Đợt L5, không phải do C2/C3 tạo ra) — không backfill toàn bộ namespace trong đợt này vì ngoài phạm vi.
