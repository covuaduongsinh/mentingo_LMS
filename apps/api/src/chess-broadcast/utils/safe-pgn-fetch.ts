...
const USER_AGENT = "MentingoChessBroadcast/1.0 (+https://covuahocduong.com)";

export class BroadcastPgnFetchError extends Error {}
...
const fetchOnce = (targetUrl: URL): Promise<FetchOnceResult> =>
  new Promise((resolve, reject) => {
    ...
    if (isIP(targetUrl.hostname) && isBlockedIpAddress(targetUrl.hostname)) {
      reject(new BroadcastPgnFetchError("Address not allowed"));
      return;
    }
    ...
    req.on("timeout", () => req.destroy(new BroadcastPgnFetchError("Request timed out")));
    req.on("error", (err) =>
      reject(err instanceof Error ? err : new BroadcastPgnFetchError(String(err))),
    );
    req.end();
  });

/**
 * Fetches at most 512KB of raw PGN text from an http(s) URL, following at most
 * 3 redirects, with every hop (including redirects) validated against the SSRF
 * blocklist. Throws BroadcastPgnFetchError on any disallowed target, timeout,
 * or network error.
 */
export const fetchPgnSafely = async (initialUrl: string): Promise<string> => {
  let currentUrl: URL;

  try {
    currentUrl = new URL(initialUrl);
  } catch {
    throw new BroadcastPgnFetchError("Invalid URL");
  }

  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
    if (currentUrl.protocol !== "http:" && currentUrl.protocol !== "https:") {
      throw new BroadcastPgnFetchError("Unsupported protocol");
    }
    ...
    if (result.location) {
      if (redirectCount === MAX_REDIRECTS) {
        throw new BroadcastPgnFetchError("Too many redirects");
      }
