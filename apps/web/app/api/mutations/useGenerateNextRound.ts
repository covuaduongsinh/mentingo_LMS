...
export function useGenerateNextRound(tournamentId: string) {
  ...
  return useMutation({
    mutationFn: async () => {
      const response = await ApiClient.api.chessTournamentControllerGenerateNextRound(tournamentId);
    ...
    onError: (error) => {
      toast({
        ...
        description: getTranslatedApiErrorMessage(
          error,
          t,
          t("chessTournament.toast.nextRoundFailed", {
            defaultValue: "Could not generate the next round",
          }),
        ),
      });
    },
